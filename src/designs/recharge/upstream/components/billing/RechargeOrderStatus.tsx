import { Check, Circle, Clock3, LoaderCircle, RefreshCw, ShieldAlert, ShieldCheck } from 'lucide-react'
...
function statusCopy(status: BillingTopupOrderStatus | null) {
  ...
  if (status.status === 'manual_review') return {
    title: pageText('billing.rechargeOrderStatus.manualReviewRequired'), detail: pageText('billing.rechargeOrderStatus.noCreditWasAddedDoNotSendAnotherTransfer'), tone: 'warning',
  } as const
  ...
  if (status.status === 'submitted' || status.status === 'confirming') return {
    title: pageText('billing.rechargeOrderStatus.verifyingTaoPayment'), detail: pageText('billing.rechargeOrderStatus.waitingForCanonicalFinalizedChainEvidenceAndTheAtomic'), tone: 'pending',
  } as const
  return {
...
function completedSteps(status: BillingTopupOrderStatus | null) {
  ...
  if (status.paymentStatus === 'finalized' || status.paymentStatus === 'succeeded') return 3
  if (status.status === 'submitted' || status.status === 'confirming') return 2
  return 1
}

export function RechargeOrderStatus({ orderID, status, error, loading, canStartAnother, onRefresh, onStartAnother }: {
  orderID: string
  ...
  return (
    <section className={`recharge-order-status recharge-order-status--${copy.tone} cs-sec`} aria-labelledby="recharge-order-status-title" aria-busy={loading}>
      ...
      <div className="recharge-order-status__summary">
        ...
        {status ? <><span><small>{pageText('billing.rechargeOrderStatus.pay')}</small><strong>{formatBillingCreditUSD(status.paidMicros)}</strong></span><span><small>{pageText('billing.rechargeOrderStatus.credit')}</small><strong>{formatBillingCreditUSD(status.creditedMicros)}</strong></span></> : null}
      </div>
