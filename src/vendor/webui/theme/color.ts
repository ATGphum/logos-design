...
export function generateNeutralScale(seed: HexColor, isDark: boolean, ink?: HexColor): HexColor[] {
  if (ink) {
    ...
    const lift = (tone: number) =>
      oklchToHex({ l: base.l + (1 - base.l) * tone, c: base.c * Math.max(0, 1 - tone), h: base.h })
    const sink = (tone: number) =>
      oklchToHex({ l: base.l * (1 - tone), c: base.c * Math.max(0, 1 - tone * (isDark ? 0.12 : 0.3)), h: base.h })
    const bg = isDark
      ? sink(clamp(0.19 + Math.max(0, base.l - 0.12) * 0.33 + base.c * 1.95, 0.17, 0.27))
      : base.l < 0.82 ? lift(0.86) : lift(clamp(0.1 + base.c * 3.2 + Math.max(0, 0.95 - base.l) * 0.35, 0.1, 0.28))
    const steps = isDark
...
export function mixColors(color1: HexColor, color2: HexColor, amount: number): HexColor {
  ...
  return oklchToHex({ l: c1.l + (c2.l - c1.l) * amount, c: c1.c + (c2.c - c1.c) * amount, h: c1.h + delta * amount })
}

export function shift(color: HexColor, value: { l?: number; c?: number; h?: number }): HexColor {
  const base = hexToOklch(color)
  return oklchToHex({ l: base.l + (value.l ?? 0), c: base.c * (value.c ?? 1), h: base.h + (value.h ?? 0) })
}
