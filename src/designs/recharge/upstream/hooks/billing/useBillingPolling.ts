...
import { parseBillingOrderStatus, type BillingOrderStatus } from '../../billingTypes'
import { billingOrderPollingDecision, billingPollingDefaults } from './billingPollingPolicy'
import { pageText } from '../../i18n/pageText'

export type BillingPollingOptions = Readonly<{
...
export function useBillingPolling(orderID: string | null, enabled = true, options: BillingPollingOptions = {}) {
  ...
  const poll = useCallback(async () => {
    ...
    if (Date.now() - startedAt.current >= maxDurationMs) {
      ...
      setLoading(false)
      setError(pageText('dynamic.billing.paymentPollingPaused'))
      return
    }
    ...
    try {
      ...
      const parsed = parseBillingOrderStatus(payload)
      if (parsed === null || parsed.id !== orderID) throw new Error(pageText('dynamic.billing.paymentStatusUnavailable'))
      ...
      setError('')
      setLoading(false)
      if (billingOrderPollingDecision(parsed) === 'continue') {
        schedule(pendingIntervalMs)
      } else {
        stopped.current = true
      }
    } catch (pollError) {
      ...
      active.current = null
      setLoading(false)
      setError(apiErrorMessage(pollError, pageText('dynamic.billing.paymentStatusUnavailable')))
  ...
  return { status, error, loading, refresh, stop }
}
